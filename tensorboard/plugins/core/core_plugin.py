...
''')
    ...
    parser.add_argument(
        ...
        choices=['user', 'design'],
        help='''\
Added in NIPGBoard, when setting --mode=design, the feature windows cannot be
used but instead they are can be resized and moved to the desired position,
and the layout of these windows can be saved with the "save layout" button on
the top right. In --mode=user (which is the default), the windows are no longer
movable or resizable but are fully functional plugins.\
''')
