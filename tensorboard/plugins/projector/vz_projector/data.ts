...
function randomSeed(seed: number): () => number {
  let nextSeed = seed;
  return function() {
    const x = Math.sin(nextSeed++) * 1000;
    return x - Math.floor(x);
  };
}

Math.random = randomSeed(43);
// console.log(rand()); // 0.05236320797738143
// console.log(rand()); // 0.9218904300764135
// console.log(rand()); // 0.6575426595271527
...
export class DataSet {
  ...
  projectPCA(): Promise<void> {
    ...
    if (this.projections['pca-0'] != null) {
      this.mode = "PCA";
      (document.querySelector("vz-projector") as any).projectionMode = "PCA";
      return Promise.resolve<void>(null);
    }
    return util.runAsyncTask('Computing PCA...', () => {
    //TODO: download after pca is computed
      ...
      this.mode = "PCA";
      (document.querySelector("vz-projector") as any).projectionMode = "PCA";
          // alert(Object.keys(this.points[0].projections));
          //  alert(this.points.length);
          // alert(this.points[0].projections['pca-0']);
    });
  }
  ...
  projectTSNE(
    ...
    let step = () => {
      if (this.tSNEShouldStop) {
        this.mode = "TSNE";
        (document.querySelector("vz-projector") as any).projectionMode = "TSNE";
        this.projections['tsne'] = false;
        stepCallback(null);
        this.tsne = null;
        this.hasTSNERun = false;

        //TODO: download t-sne projections
//        alert(this.points[0].projections['tsne-0']);
//          
//        let numOfPoints = this.points.length;
//        let tsneVectors: number[][] = new Array(numOfPoints);
//        let labels: string [] = new Array(numOfPoints);
//        let NUM_TSNE_DIM = this.tsne.getDim();
//
//        for (let i=0; i< numOfPoints; i++){
//            let newV = new Array(NUM_TSNE_DIM);
//            for (let d = 0; d < NUM_TSNE_DIM; d++){
//                let label = 'tsne-' + d;
//                newV[d] = this.points[i].projections[label];
//        }
//            
//         tsneVectors[i] = newV;
//        //alert(points[i].metadata.Label);
//         labels[i] = this.points[i].metadata.Label.toString();
//        }
      ...
      const step = () => {
        ...
        util.runAsyncTask(progressMsg, () => {
          if (currentEpoch < nEpochs) {
            ...
            this.mode = "UMAP";
            (document.querySelector("vz-projector") as any).projectionMode = "UMAP";
