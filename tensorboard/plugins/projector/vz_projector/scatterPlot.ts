...
export class ScatterPlot {
  ...
  public addPosMode = false;
  public addNegMode = false;
  public removeMode = false;
  public groupMode = true;
  public clusterMode = false;
  public outlierMode = false;
  private addPosFlag = false;
  private addNegFlag = false;
  ...
  private addInteractionListeners() {
    ...
    this.container.addEventListener('click', this.clickModified.bind(this));
  ...
  private clickModified(e?: MouseEvent, notify = true) {
  ...
  private onClick(e?: MouseEvent, notify = true) {
    ...
    if (!this.isDragSequence && notify) {
      ...
      if (this.groupMode) {
        //This is the N^2 Selection case, when we want to dynamically assign a lot of pairs to one
        //predetermined focus point. This is the default, the N Selection case is deprecated.
        //I.E: Clicking points Y, W, Z creates pairs X<->Y, X<->W, X<->Z; where X is clicked before.

        if (this.addPosMode) {
          //Adding a positive pair to this.firstPoint.
          if ((selection.length != 0) && (this.firstPoint) && (selection[0] != this.firstPoint[0])) {
            window.dispatchEvent(new CustomEvent("addPos", {detail: {
              first: this.firstPoint,
              second: selection
            }}))
          }
        } else if (this.addNegMode) {
          //Adding a negative pair to this.firstPoint.
          if ((selection.length != 0) && (this.firstPoint) && (selection[0] != this.firstPoint[0])) {
            window.dispatchEvent(new CustomEvent("addNeg", {detail: {
              first: this.firstPoint,
              second: selection
            }}))
          }
        } else if (this.removeMode) {
          //Deleting a pair from this.firstPoint.
          if ((selection.length != 0) && (this.firstPoint) && (selection[0] != this.firstPoint[0])) {
            window.dispatchEvent(new CustomEvent("removeP", {detail: {
              first: this.firstPoint,
              second: selection
            }}))
          }
        } else if (this.clusterMode) {
          if(this.firstPoint && selection) {
            projector.cluster1 = this.firstPoint[0];
            projector.cluster2 = selection[0];
          }
        } else if (this.outlierMode) {
          projector.outlier = selection[0];
        } else {
          this.firstPoint = (selection.length != 0) ? selection : undefined;
          this.projectorEventContext.notifySelectionChanged(selection);
        }
      } else {
        //This is the deprecated N Selection case where you individuall add any X-Y pair independent
        //of any focus point.
        //I.E: Clicking points X,Y,W,Z creates pairs X<->Y and W<->Z

        if (this.addPosMode) {
          //Adding a positive pair to this.firstPoint.

          this.addNegFlag = false;
          this.addNegMode = false;
          this.removeFlag = false;
          this.removeMode = false;
          if (selection.length != 0) {
            if (!(this.addPosFlag)) {
              this.firstPoint = selection;
              //projector.setInfo("Select the second item in the positive relation!");
              tf_tensorboard.notifyHelpChanged("Select the second item in the positive relation!","projector");
            } else if ((this.firstPoint) && (selection[0] != this.firstPoint[0])) {
              window.dispatchEvent(new CustomEvent("addPos", {detail: {
                first: this.firstPoint,
                second: selection
              }}))
              this.firstPoint = undefined;
              //projector.setInfo("Added! Select another first item in the positive relation!");
              tf_tensorboard.notifyHelpChanged("Added! Select another first item in the positive relation!","projector");
            }
            this.addPosFlag = !this.addPosFlag;
          }
        } else if (this.addNegMode) {
          //Adding a negative pair to this.firstPoint.
          
          this.addPosFlag = false;
          this.addPosMode = false;
          this.removeFlag = false;
          this.removeMode = false;
          if (selection.length != 0) {
            if (!(this.addNegFlag)) {
              this.firstPoint = selection;
              //projector.setInfo("Select the second item in the negative relation!");
              tf_tensorboard.notifyHelpChanged("Select the second item in the negative relation!","projector");
            } else if ((this.firstPoint) && (selection[0] != this.firstPoint[0])) {
              window.dispatchEvent(new CustomEvent("addNeg", {detail: {
                first: this.firstPoint,
                second: selection
              ...
              this.firstPoint = undefined;
              //projector.setInfo("Added! Select another first item in the negative relation!");
              tf_tensorboard.notifyHelpChanged("Added! Select another first item in the negative relation!","projector");
            }
            this.addNegFlag = !this.addNegFlag;
          }
        } else if (this.removeMode) {
          //Deleting a pair from this.firstPoint.
          this.addPosFlag = false;
          ...
          if (selection.length != 0) {
            if (!(this.removeFlag)) {
              this.firstPoint = selection;
              //projector.setInfo("Select the second item in relation you want to delete!");
              tf_tensorboard.notifyHelpChanged("Select the second item in relation you want to delete!","projector");
            } else if ((this.firstPoint) && (selection[0] != this.firstPoint[0])) {
              window.dispatchEvent(new CustomEvent("removeP", {detail: {
                first: this.firstPoint,
                second: selection
              ...
              this.firstPoint = undefined;
              //projector.setInfo("Deleted! Selected another first item in a pair to delete!");
              tf_tensorboard.notifyHelpChanged("Deleted! Selected another first item in a pair to delete!","projector");
            }
            this.removeFlag = !this.removeFlag;
          }
        } else {
          this.addPosFlag = this.addNegFlag = this.removeFlag = false;
        }
  ...
  private onMouseDown(e: MouseEvent) {
    ...
    if ((!this.selecting) && (this.nearestPoint != null)) {
      this.onClick(e);
    }
    if (this.selecting) {
